...
my $sthmic = $dbh->prepare("SELECT Titulo, Autores, Fecha FROM micro WHERE Pmid=? ");
unless ( defined ($sthmic) ) {die "no se pueden prepara las datos para insertarlos en las tablas\n";}

my $sthupd = '';
####################### CREAR EL FASTA PARA HACE EL BLAST
open (FASTA, '+>outfasta2')||die "ERROR: no se puede leer o crear el archivo fasta\n";
			...
			if ( $line =~ /^\/\// ){    #cuando acabe cada entrada (con //) guarda los datos en la BD
				...
				if ($gen =~ /(16S\s*.*)|([Ss]mall\s+subunit\s+ribosomal\s+RNA)/){
					if (length $seq > 200 && length $seq < 1800){
						...
						unless ( $sthmic->execute($pmid) ) {die "Se ha producido un problema al conectar con la base de datos: " . $DBI::errstr unless ( defined($dbh) );}
					if ($sthmic->rows == 0) {	# En caso de que no exista la entrada en la BD, la introduce como nueva
						print FASTA ">$pmid\n$seq\n\n";
						print TITULO "$titaut\n";
					...
					else {	#En caso de que la entrada exista, comprueba la existencia de alguna actualización
						while ( ($tit, $aut, $fech) = $sthmic->fetchrow_array() ) {
							if ($tit ne $titulo || $aut ne $autores || $fech ne $fecha){
								$sthupd = $dbh->prepare("UPDATE micro set Titulo=$tit, Autores=$aut, Fecha=$fech WHERE Pmid=$pmid ");
unless ( defined ($sthupd) ) {die "no se pueden prepara las datos para insertarlos en las tablas\n";}
							}
						}
